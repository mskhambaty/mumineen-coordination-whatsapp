...
Recommended on hosts that run OpenClaw: point `CAPTURE_DIR` to
`~/.openclaw/data/captures` so OpenClaw-side jobs/prompts can read captures
without crossing directory boundaries.

---

## Distinct phone extraction (per JSONL file)

This repo includes a deterministic extractor for downstream jobs and OpenClaw
prompts:

```bash
# Human-readable output
npm run extract:phones -- --capture-dir /home/asc/claw-message-ingester/captures
...
- Scans all `*.jsonl` files in the capture directory.
- Uses only `sender_phone` from each row.
- Normalizes phone values by removing any `@...` suffix and retaining digits.
- Emits distinct numbers grouped by department and WhatsApp group (using
  `groups.json` when available).

Prompt template for OpenClaw is available at
`extract-phones.prompt.md`.

### OpenClaw prompt execution

Execute the instructions in `extract-phones.prompt.md`.

The prompt runs the extraction command and returns a markdown table grouped by:

- department
- group_jid
- wa_subject
- distinct phone count and numbers
...
| `extract-distinct-phones.js` | Deterministic phone extractor grouped by department/group. |
| `extract-phones.prompt.md` | OpenClaw prompt instructions to execute extraction and format output. |
...
Avoid empty-string entries such as `allowFrom: [""]` or `groupAllowFrom: [""]`.
They are ambiguous across builds (ignored, deny-all, or schema-invalid). Use
explicit valid values, an empty list (`[]`) when intentionally denying all, or
omit the key entirely when this architecture says to leave it out.
