...
- **Cancel / Reopen / Family not attending** (`POST /api/admin/mumineen/registration`): from the lookup results, registered families show **Cancel**, cancelled families show **Reopen**, and an **unregistered head-of-family** row shows **Family not attending** — which registers the family (`status=submitted`) and flags every member `not_attending`, so they drop out of the unregistered nudge and all attendance rollups.
