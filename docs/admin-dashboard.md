...
- **Number focus (Conversations tab)** — a **Helpline / Survey / All** selector (default **Helpline**) cuts broadcast noise. A niyaz broadcast (RSVP/feedback) sent to thousands creates/updates each recipient's thread; **Survey** = "Survey-only" conversations (the person *never texted the helpline* — `has_helpline_activity` false, i.e. no inbound on a non-broadcast number), **Helpline** = everyone who did, **All** = everything. Crucially this keys off *helpline activity*, **not the latest message** — so a real helpline conversation stays in Helpline even after an RSVP broadcast is later sent to it (it doesn't disappear). The flag is computed server-side over all messages (not the loaded window). The selector only appears when a broadcast account is configured. Also filters the Conversations KPI counts.
