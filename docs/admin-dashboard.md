...
- **Number focus (Conversations tab)** — a **Helpline / Survey / All** selector (default **Helpline**) cuts broadcast noise. A niyaz broadcast (RSVP/feedback) sent to thousands bumps each recipient's thread to the top with a template preview; **Helpline** hides threads whose most-recent message is on the broadcast/Survey number, **Survey** shows only those, **All** shows everything. Classified by the latest message's `phone_number_id` (the session number is unreliable — often `null` even for broadcast-topped threads). The selector only appears when a broadcast account is configured. Also filters the Conversations KPI counts.
