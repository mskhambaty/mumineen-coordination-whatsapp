...
- **Broadcast noise filtering (Conversations tab)** — cuts RSVP/feedback broadcast noise by classifying **messages** as *broadcast/automated* vs *conversational*, rather than by phone number. A message is broadcast/automated when it's a **template send** (`raw_payload.template`, body `[template:…]` — RSVP/feedback/digests/notifications), an **RSVP/feedback button or flow response** (`message_type` `interactive`/`button`), or a **system broadcast** (`raw_payload.source` in `niyaz_rsvp_ended`/`issue_close_broadcast`); see `isBroadcastMessage` (client) mirrored by the server.
  ...
  - **In the list:** a **Show: Conversations / Broadcast-only / All** selector (default **Conversations**) hides threads with *no* conversational message (pure broadcast recipients). Backed by the server-computed `has_conversational_message` over **all** messages (not the loaded window), so a real conversation never gets hidden because a broadcast was later sent to it. Also filters the Conversations KPI counts.
