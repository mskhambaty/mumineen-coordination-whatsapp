...
- **Broadcast noise filtering (Conversations tab)** — cuts RSVP/feedback broadcast noise by classifying **messages** as *broadcast/automated* vs *conversational*, rather than by phone number. A message is broadcast/automated when it's a **template send** (`raw_payload.template`, body `[template:…]` — RSVP/feedback/digests/notifications), an **RSVP/feedback button or flow response** (`message_type` `interactive`/`button`), or a **system broadcast text** (`raw_payload.source` in `niyaz_rsvp_ended`/`issue_close_broadcast` — e.g. the "Shukran for your reply. RSVP for…" note, which has no template key but isn't real conversation); see `isBroadcastMessage` (client), **exactly mirrored** by the server's `has_conversational_message` / preview queries (all three conditions) so the in-thread hide and the list bucketing agree.
  - **In a thread:** a **"Hide broadcast / RSVP messages"** toggle (default **on**) hides those from the message pane so a monitor sees only the real exchange; the chip shows the hidden count and toggles back.
  - **In the list:** a **Show: Conversations / Broadcast-only / All** selector (default **Conversations**) hides Survey/broadcast threads. A thread is Survey/broadcast when it has *no* conversational message (`has_conversational_message` false — pure broadcast recipient) **or** its agent activity is **RSVP-only** (`is_rsvp_only` — used an RSVP tool `get/set_family_meal_rsvps` and no other substantive tool; an RSVP status exchange has real text but is RSVP-topic). A thread that used a non-RSVP tool (e.g. `get_lisan_word_meaning` for "meaning of tawakul") stays a Conversation. Both flags are server-computed over **all** data, so a real conversation never gets hidden because a broadcast reached it. Also filters the Conversations KPI counts.
  - The Conversations tab surfaces **real conversations on any number**, including ones the agent had on the broadcast number (e.g. RSVP questions that turned into a real chat): the `main`-scoped recent query excludes the broadcast number, so a supplementary query pulls the phones with the most-recent *conversational* messages and loads their sessions regardless of scope/window. (The `niyaz` scope view stays broadcast-only.)
  - On the Conversations tab the row **preview, timestamp, and sort use the last *conversational* message** (`conversational_last_message`, server-computed), not a later broadcast — so a thread that last had a real exchange on Jun 15 shows/sorts by Jun 15 even if an RSVP broadcast hit it on Jun 17. Broadcast-only threads fall back to their latest (broadcast) message.
...
- **Live updates via SSE** — the inbox subscribes to `GET /api/admin/conversations/stream` (EventSource) and refetches only when the server signals new activity, instead of polling on a timer. Background refreshes **preserve the open thread** (`preserveSelectedThread`): the list payload only carries recent-capped messages, so a silent refresh keeps the full thread the user loaded (e.g. while scrolling history on mobile) and merely *appends* genuinely-new messages — it never snaps the thread back to recent-only.
...
- **VIP · Category / Idara** (Mumineen section): identifies VIP mumin — any roster member with a special `mumineen.category` tier (e.g. *Baite Zainy*, *Qasre Aali*) **or** a qualifying `mumineen.idara` (institutional roles — *Attalimiyah*, *Aljamea KHDGZ*, *Ummal Kiram*, …; see `VIP_IDARAS` in [`src/lib/registration/vip.ts`](../src/lib/registration/vip.ts)). The two fold into one **VIP group** dimension via `vipGroup()` — a non-blank category wins over idara, so a member tagged both is counted once. A trailing parenthetical qualifier on the idara is stripped before matching, so workshop/variant tags like *Attalimiyah (WH)* fold into their base idara (*Attalimiyah*). Grouped with a clickable bar per VIP group. Counts **all VIPs regardless of attendance** (VIP/protocol status is a roster attribute), still respecting the page's Local/Mehman, Status, and Gender filters. Each bar drills into that group (`segment=vip&value=…`). A card-level **Export all VIPs** button (leadership/PM-HOD only, like all drill-downs) downloads every VIP across groups in one CSV with a leading `VIP Group` column, sorted group-by-group; it calls the detail route with `segment=vip` and no `value`.
