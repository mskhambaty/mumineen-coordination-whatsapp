...
- **VIP · Category** (Mumineen section): identifies VIP mumin — any roster member with a non-blank `mumineen.category` (e.g. *Baite Zainy*, *Qasre Aali*) — grouped by category value with a clickable bar per category. Counts **all VIPs regardless of attendance** (VIP/protocol status is a roster attribute), still respecting the page's Local/Mehman, Status, and Gender filters. Each bar drills into that category (`segment=category&value=…`). A card-level **Export all VIPs** button (leadership/PM-HOD only, like all drill-downs) downloads every VIP across categories in one CSV with a leading `Category` column, sorted category-by-category; it calls the detail route with `segment=category` and no `value`.
