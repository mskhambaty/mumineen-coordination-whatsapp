...
| `WHATSAPP_DRAIN_BATCH_SIZE` | — | `5` | Default broadcast send-throttle batch: recipients the drain sends per minute-cron tick. The Send Templates console pre-fills this and the drain falls back to it when a broadcast has no per-send override. Keep gentle to avoid Meta's spam/throughput limit (error 131048); raise once sends stop getting rate-limited. |
| `WHATSAPP_SEND_INTERVAL_MS` | — | `2000` | Default broadcast send-throttle delay (ms) between individual sends within a batch. Same pre-fill/fallback behavior as `WHATSAPP_DRAIN_BATCH_SIZE`. Per-send delay is the primary anti-spam lever — bursting is the main 131048 trigger. |
