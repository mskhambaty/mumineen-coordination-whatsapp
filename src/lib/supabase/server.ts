...
// True if the user is an active member of any department (i.e., an internal team member).
export async function isDepartmentMember(userId: string): Promise<boolean> {
  if (!userId) return false;
  const { data, error } = await getSupabaseAdmin()
    .from("department_members")
    .select("id")
...
// True if the user is an active member of the Accommodations department (any dept_role).
export async function isAccommodationsMember(userId: string): Promise<boolean> {
  if (!userId) return false;
  const { data, error } = await getSupabaseAdmin()
    .from("department_members")
    .select("id, departments!inner(name)")
    .eq("user_id", userId)
    .eq("is_active", true)
    .eq("departments.name", "Accommodations")
    .limit(1)
    .maybeSingle();
  if (error) return false;
  return Boolean(data);
