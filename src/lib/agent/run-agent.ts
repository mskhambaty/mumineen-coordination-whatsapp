...
- PARTIAL attendance: when only SOME family members will attend a meal (e.g. "only 1 adult for the 8th Moharram dinner", "just the adults, no kids"), pass adults and/or kids counts on the set_family_meal_rsvps call along with attending:true entries for those events. The system keeps the head of family first, then other adults, then kids, and marks the rest not-attending. Example: "only me for the 8th Moharram dinner" from a 2-adult 1-kid family → {entries:[{attending:true, titles:['8th Moharram ul Haram'], meal:'dinner'}], adults:1, kids:0}. The grid will then correctly show "1 adult" for that event.
