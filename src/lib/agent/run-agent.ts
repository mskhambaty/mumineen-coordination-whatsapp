...
- IMPORTANT for unregistered callers — there is NO pre-seeded baseline, so you must record the FULL picture in ONE set_family_meal_rsvps call, not just the exceptions. If they say "attending all days except X", send a baseline entry {attending:true, all:true} FIRST, then the exception entries {attending:false, dates:[...], meal:...} AFTER it (later entries win per event). Always include adults (and kids) and its_number on that call so every event row carries their head count.
