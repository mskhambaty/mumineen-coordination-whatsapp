...
const requireAdminLeadership = vi.fn();
...
vi.mock("@/lib/api/auth", () => ({
  requireAdminLeadership: (...a: unknown[]) => requireAdminLeadership(...a),
}));
...
function req(body: unknown): NextRequest {
  return new NextRequest("http://localhost/x", {
    method: "POST",
    headers: { "content-type": "application/json", "x-admin-key": "k", "x-portal-user-id": "u1" },
...
describe("preview route auth + behavior", () => {
  it("denies a non admin/leadership caller (401)", async () => {
    requireAdminLeadership.mockResolvedValue(false);
  ...
  it("rejects an invalid audience key (400)", async () => {
    requireAdminLeadership.mockResolvedValue(true);
