...
export default function AccommodationsPage() {
  ...
  // Auth gate — compute canWrite outside of effect to avoid setState-in-effect lint error.
  const canWrite = (() => {
  ...
  const fetchSuggestions = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/admin/accommodations/matches?action=suggest", { headers: headers() });
      if (!res.ok) throw new Error(await res.text());
      const data = await res.json();
  ...
  useEffect(() => {
    ...
    else if (tab === "suggest") fetchSuggestions();
  }, [tab, fetchHosts, fetchGuests, fetchMatches, fetchSuggestions]);
      ...
      {tab === "hosts" && !loading && (
        <div>
          ...
          <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
            ...
            Allocated: {hosts.reduce((s, h) => s + h.confirmed_allocated, 0)} |
      ...
      {tab === "suggest" && !loading && (
        <div>
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">
            Ranked guest-host pairing suggestions. Only unmatched guests with eligible hosts shown.
          </p>
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-collapse">
              <thead>
              ...
              <tbody>
                {suggestions.slice(0, 100).map((s, i) => (
                  <tr key={`${s.guest.family_id}-${s.host.id}-${i}`} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
                    <td className="p-2 font-mono font-semibold">{s.score}</td>
                    <td className="p-2">
                      <span className="font-medium">{s.guest.head_name ?? s.guest.hof_its}</span>
                      <span className="text-xs text-gray-400 ml-1">({s.guest.member_count} ppl)</span>
                    </td>
                    <td className="p-2 text-center">{s.guest.member_count}</td>
                    <td className="p-2">{s.host.display_name}</td>
                    <td className="p-2 text-center text-green-600 dark:text-green-400">{s.host.remaining_capacity}</td>
            ...
            {suggestions.length === 0 && <p className="text-gray-500 dark:text-gray-400 p-4 text-center">No suggestions available (all guests matched or no hosts imported).</p>}
            {suggestions.length > 100 && <p className="text-xs text-gray-400 mt-2">Showing top 100 of {suggestions.length} suggestions.</p>}
          </div>
        </div>
      )}
    </div>
