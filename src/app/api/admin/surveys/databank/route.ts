...
export async function GET(req: NextRequest) {
  ...
  const [{ data: sections }, { data: questions }, { data: groups }] = await Promise.all([
    ...
    supabase.from("survey_questions").select("id, section_id, text, type, options, negative_values, polarity, is_general, collect_comment, comment_threshold, required, scored, sort_order, active").eq("active", true).order("sort_order"),
