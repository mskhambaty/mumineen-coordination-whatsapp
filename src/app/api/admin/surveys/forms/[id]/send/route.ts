...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const body = (await req.json().catch(() => ({}))) as { template?: unknown; freeWindowOnly?: unknown; excludeAlreadySent?: unknown };
  ...
  const result = await commitAndSendForm(id, template, freeWindowOnly, excludeAlreadySent);
