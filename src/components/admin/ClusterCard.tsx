...
export default function ClusterCard({
  ...
  if (state === "done" && result) {
    return (
      <div className="flex items-center gap-3 rounded-xl border border-emerald-600 bg-emerald-900/40 px-4 py-3">
        {/* Checkmark icon */}
        <svg
  ...
  if (state === "collapsed") {
    return (
      <button
        ...
        className="flex w-full items-center gap-3 rounded-xl border border-gray-700 bg-gray-900 px-4 py-3 text-left transition-colors hover:border-gray-600 hover:bg-gray-800/80"
      >
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-semibold text-gray-100">{suggestedTitle}</p>
  ...
  return (
    <div className="rounded-xl border border-gray-700 bg-gray-900">
      ...
      >
        ...
        <div className="flex shrink-0 items-center gap-2">
          {state === "reviewing" ? (
            <span className="rounded-full border border-purple-700/60 bg-purple-900/60 px-2 py-0.5 text-[11px] font-medium uppercase tracking-wider text-purple-300">
              REVIEWING
            </span>
          ) : (
            <PriorityBadge priority={suggestedPriority} />
      ...
      <div className="border-t border-gray-700/60 px-4 pb-4 pt-3 space-y-3">
        ...
        <div className="space-y-1">
          {escalations.map((esc) => {
            ...
            return (
              <label
                key={esc.session_id}
                className={`flex cursor-pointer items-center gap-3 rounded-lg px-2 py-1.5 transition-colors hover:bg-gray-800/60 ${
              ...
              >
                <input
                  ...
                  className="h-4 w-4 shrink-0 rounded border-gray-600 bg-gray-800 text-purple-600 focus:ring-purple-500"
                />
                <span
                  className={`min-w-[90px] shrink-0 text-sm font-medium text-gray-200 ${
                ...
                {esc.last_message_preview && (
                  <span
                    className={`flex-1 truncate text-xs text-gray-400 ${
                      isChecked ? "" : "line-through"
                    }`}
                  >
        ...
        {/* AI reasoning */}
        <div className="rounded-lg border border-purple-800/60 bg-purple-950/40 px-3 py-2.5">
