-- Dynamic roster import support.
--
-- 1) get_mumineen_columns(): lets the importer read the live mumineen schema so it can
--    auto-map any uploaded sheet header that exactly matches a (non-generated) column name.
--    New roster columns added to the table are then picked up with zero code changes.
--
-- 2) finalize_mumineen_import(): adds p_deactivate_missing so the app can request an
--    additive import (a brand-new batch with no overlap) that preserves the existing roster
--    instead of soft-deactivating everyone not in the uploaded file.

create or replace function public.get_mumineen_columns()
...
create or replace function public.finalize_mumineen_import(
  p_its text[],
  p_hof text[],
